...
# Main
diag 'Fuzzing: infix type signature parser (via sizeof / _is_type)';
...
for my $i ( 1 .. $max_iter ) {
    ...
    my $is_valid;
    my $mode = rand(1);
    if ( $mode < 0.35 ) {    # Generate a fresh valid sig
        $sig      = gen_valid_sig();
        $is_valid = 1;
    }
    elsif ( $mode < 0.55 ) {    # Generate a valid function sig
        $sig      = gen_func_sig();
        $is_valid = 1;
    }
    elsif ( $mode < 0.75 ) {    # Mutate a previously valid sig
        if (@history) {
    ...
    else {    # Generate a deliberately malformed sig
        $sig      = gen_malformed_sig();
        $is_valid = 0;
    }
    push @history, $sig if defined $sig && $sig ne '';
    @history = @history[ -30 .. -1 ]                                                             if @history > 30;
    note 'Sig[' . ( $is_valid ? 'V' : 'M' ) . ']: [' . ( defined $sig ? $sig : '<undef>' ) . ']' if $verbose;
