...
=item B<pin( $var, $lib, $symbol, $type )> — Binds to an exported symbol. This is the most common form.
...
=item B<pin( $var, $address, $type )> — Binds to a raw memory address (e.g., from C<find_symbol> or pointer arithmetic).

    my $addr = address($some_ptr);
    pin my $val, $addr, Int;

=item B<pin( $var, $existing_pin )> — Clones the binding from an existing pin (copies the address and type).
...
In C, C<const> is a contract. Affix enforces this contract at the Perl level using magical VTables. If a memory block
is marked as readonly, any attempt to modify it from Perl will throw a fatal exception: C<Modification of a read-only C
value attempted>.
