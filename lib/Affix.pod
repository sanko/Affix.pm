...
=item 3. B<Smart Enums>: Generating Perl constants in your package and enabling dualvar marshalling (string names and integer values).

=back

=head2 C<attach_destructor( $pin, $destructor, [$lib] )>

    my $p = malloc(16);
    my $free_ptr = find_symbol(libc(), 'free');
    attach_destructor($p, $free_ptr, libc());

Attaches a custom C function as a destructor to a managed pin. When the pin goes out of scope, the destructor will be
called with the pointer address as its only argument.

This is useful for C libraries that require specific cleanup functions (e.g. C<SDL_DestroyWindow>, C<free>, etc.).

=head2 C<ThisCall( $callback_or_signature )>

    # Prepend 'this' pointer to a callback signature
    my $cb = ThisCall Callback[ [Int] => Void ];
    # signature is now: *((*void,int)->void)

    # Also works on raw signatures
    my $sig = ThisCall '*((int)->void)';

A helper for C++ C<__thiscall> style callbacks. It prepends a C<Pointer[Void]> (the C<this> pointer) to the argument
list of a callback definition or signature string.
...
For manual memory management, use C<malloc>, C<calloc>, or C<cast>. These return B<Pins>, which are objects of the
C<Affix::Pointer> class. A Pin is a reference to a scalar holding the memory address, blessed with magic and overloaded
...
=head4 C<Affix::Pointer> Methods

=over

=item C<address()>

Returns the virtual memory address as a C<UInt64>.

    say $ptr->address;

=item C<type()>

Returns the L<infix|https://github.com/sanko/infix/> signature of the pointer type.
...
Returns the number of elements if the pin is an C<Array> type. For C<Void> pointers, returns the byte size if known.
...
Reinterprets the pointer as a new type. See L<C<cast( ... )>|/cast( $ptr, $type )>.

=back

=head4 Array Indexing

C<Affix::Pointer> objects overload array dereferencing (C<@$ptr>). This allows you to use standard Perl array syntax to
access C memory without manual casting or pointer arithmetic.

    my $array = calloc(10, Double);
    $array->[5] = 3.14;
    say $array->[5];

For C<Void> pointers, indexing is byte-based (equivalent to C<char*>).
...
Standard C<Struct> returns are deep-copied into Perl hashes. Modifying the hash does not affect the original C memory.
C<LiveStruct> returns a zero-copy "live" view of the struct or union.
...
Live aggregates are blessed into the C<Affix::Live> class and implement the C<TIEHASH> interface, meaning they behave
like standard Perl hashes (C<keys>, C<each>, etc. all work).

B<Recursive Liveness:> When accessing a nested struct member within a live view, Affix automatically returns another
live view (an C<Affix::Live> hash) instead of a copy, allowing deep modifications to C memory.
...
=item B<Live Arrays (C<LiveArray[Type, N]>)>

Standard C<Array> returns are deep-copied into Perl Array References. C<LiveArray> returns a live C<Affix::Pointer>
object that allows direct indexing into C memory.
...
Callback objects returned by C<Callback[ ... ]> have a C<params()> method that returns an array reference of the
parameter type objects. This can be used to introspect or modify the callback signature before it is used.

    my $cb = Callback[ [Int] => Void ];
    push @{ $cb->params }, String; # Add an argument manually
