...
Maps a Perl filehandle (Glob or IO object) to a standard C C<FILE*>. You can pass a Perl filehandle to a C function
expecting C<FILE*>, and you can receive a C<FILE*> from C (which Affix wraps in a Perl IO handle).

    affix $lib, 'fprintf', [ File, String ] => Int;
    open my $fh, '>', 'log.txt';
    fprintf($fh, "Hello from Affix!");

=item C<PerlIO>

Similar to C<File>, but maps to the internal C<PerlIO*> stream. Use this when the C function expects C<PerlIO*>.
