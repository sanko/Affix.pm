...
Affix now features a completely reimagined memory access system using Perl's internal magic to
map Perl variables directly to native C memory. This provides zero-copy performance with the ergonomics of native Perl
Hashes and Arrays.
...
Allocates C<$size> bytes of uninitialized memory.

    my $ptr = malloc(1024); # Allocates 1KB

=head3 C<calloc( $count, $size )>

Allocates memory for an array of C<$count> elements of the given C<$size>, and zero-initializes the memory.

    my $arr = calloc( 10, sizeof(Int) );
...
=head2 Const-Correctness

When interfacing with C, it is critical to respect `const` boundaries to prevent segmentation faults or undefined behavior. Affix provides both declarative and imperative ways to protect memory.

=head3 Declarative: C<Const[ $type ]>

You can declare a type as read-only directly in the signature. Affix will enforce this protection automatically.

    typedef HardwareInfo => Struct[
        device_id   => Const[ Int ],             # Immutable hardware ID
        temperature => Float,                    # Mutable sensor reading
        name        => Array[ Const[Char], 32 ]  # Immutable string buffer
    ];

If you try to write to `$info->{device_id}`, Affix will throw a fatal exception: C<Modification of a read-only C value attempted>.

=head3 Imperative: C<readonly( $pin, [$bool] )>

Sometimes you need to dynamically lock memory, or you need an "escape hatch" (akin to C++'s `const_cast`) to bypass a `Const` signature.

    my $info = cast($mem, HardwareInfo());

    # Force bypass the Const declaration
    Affix::readonly($info->{device_id}, 0);
    $info->{device_id} = 999;

    # Re-lock the memory
    Affix::readonly($info->{device_id}, 1);

=head1 ZERO-COPY AGGREGATES

When C returns a pointer to a struct or an array, Affix does not copy the data. Instead, it returns a magical Perl variable that maps directly to the C memory.
...
Traversing a `NULL` pointer in C causes a segfault. Affix wraps C memory in Perl safety rails. If you try to traverse a `NULL` pointer inside a struct, Affix intercepts it and throws a standard Perl exception (C<Can't use an undefined value as a HASH reference>).
