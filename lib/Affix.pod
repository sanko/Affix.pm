...
Standard C<Struct> returns are deep-copied into Perl hashes. Modifying the hash does not affect the original C memory. C<LiveStruct> returns a zero-copy "live" view of the
struct or union.
...
Live aggregates are blessed into the C<Affix::Live> class and implement the C<TIEHASH> interface, meaning they
behave like standard Perl hashes (C<keys>, C<each>, etc. all work).
