...
filename; I'm giving you the raw code." We specify C<lang =E<gt> 'c'>, but C<Affix::Compiler> is a polyglot—it could
...
This allows us to pass the hockey stick emoji C<'🏒'> directly.
...
If you are typing special characters like '🏒' directly into your Perl script, always remember C<use utf8;> at the
...
to bind C<printf> (which is a variadic function—see the Variadic recipes).
...
self-documenting. It also prevents accidents—if you later define C<Car => Pointer[Void]>, Affix won't stop you from
...
Notice we used an emoji in the app name: C<'Affix 🏒'>. Because we used the C<String> type, Affix automatically
...
Affix includes a small expression parser. This allows you to copy-paste definitions from C headers—like bit shifts
(C<<< 1 << 4 >>>) or combinations (C<FLAG_A | FLAG_B>)—directly into your Affix definition without manually
