...
Some of the most fun projects, IMO, are those that play music or video. In this
section, I'll collect a few examples based around exactly that with popular
libs on various platforms.
...
something here so let's wrap L<libVLC|https://www.videolan.org/vlc/libvlc.html>
ourselves!

Here's a very rough wrapper to get you started:
    ...
    affix( 'vlc', [ 'libvlc_media_release', 'VLC::media_release' ], [ Pointer [Void] ], Int );
    affix(
        'vlc',
    ...
    affix(
        'vlc',
        [ 'libvlc_media_player_release', 'VLC::player_release' ],
        [ Pointer [Void] ], Int
    );
    affix( 'vlc', [ 'libvlc_release', 'VLC::release' ], [ Pointer [Void] ], Int );

There's a lot of nice things there (like the various 'release' functions) that
we aren't going to bother with here but your code should wrap those. Head over
to the L<VideoLAN Wiki for libVLC's
documentation|https://videolan.videolan.me/vlc/group__libvlc.html>.

Now, let's listen to the radio. WGBH is an NPR station in Boston, they stream
24 hours a day. Here's how you'd tune in:
...
it for you:
    ...
    my $vlc    = VLC::new( 0, 0 );
    my $media  = VLC::new_path( $vlc, '/home/s/Downloads/sample-1.mp3' );
    my $player = VLC::new_from_media($media);
    VLC::play($player);
    sleep 5;    # give VLC a sec to parse media asynchronously
    sleep int( ( VLC::get_length($player) / 1000 ) - 5 ); # len is in microsecs

VLC can gather play length on local files but it takes a moment. Here, we move
on as soon as the mp3 is finished playing.
