...
Optional. Explicitly select the parser driver. Values are C<'Clang'> or C<'Regex'>. If omitted, C<Affix::Bind> attempts
...
If you want to use a C library immediately without creating a separate Perl module file, use the C<wrap> method.
    ...
    Affix::Bind->new( project_files => ['demo.h'] )->wrap($lib);
...
calling C<wrap>:
    ...
    my $binder = Affix::Bind->new( project_files => ['demo.h'] );
