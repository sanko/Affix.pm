...
The preferred driver. It uses the system's C<clang> executable to dump the AST in JSON format. It is extremely accurate, handles C preprocessor logic (macros, includes), and resolves complex typedef chains correctly.
...
An array reference of paths to search for C<#include "..."> directives. The directory of every file in C<project_files> is automatically added to this list.
...
Represents a C<typedef>. Affix::Bind attempts to "resolve" typedefs. If a typedef points to an anonymous struct
defined immediately adjacent to it, Affix::Bind will merge them into a single named Affix::Bind::Struct.
