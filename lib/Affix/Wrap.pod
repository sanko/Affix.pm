...
=head2 generate( $lib, $pkg, $file )

    $binder->generate( 'mylib', 'My::Lib', 'lib/My/Lib.pm' );

A convenience method that performs the "Static Generation" workflow. It parses the headers, resolves macros, and emits
a complete Perl module file to the specified path.

The generated module will:

=over

=item 1. Declare the package C<$pkg>.

=item 2. Load L<Affix>.

=item 3. Define types, constants, and functions found in the headers.

=item 4. Attempt to bind to library C<$lib> (which is passed as a string literal to the generated code).

=back

This method handles macro resolution and ensures types are properly quoted.
