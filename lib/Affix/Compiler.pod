...
C<Affix::Compiler> is a cross-platform compilation utility designed to generate shared libraries (C<.dll>, C<.so>,
...
If you provide source files for only B<one> language, C<Affix::Compiler> delegates the entire build process to that
