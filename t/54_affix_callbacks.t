...
ok !!Affix::wrap( $lib, 'cb_dZb_b', [ CodeRef [ [ Double, Str, Bool ] => Bool ] ] => Bool )->(
    sub {
        is_deeply(
            \@_,
            [ ( $Config{usequadmath} ? 9.90000000000000035527136788005009 : $Config{uselongdouble} ? 9.90000000000000036 : 9.9 ), 'Hi', !1 ],
            '[ Double, Str, Bool ]'
        );
        return !0;
    }
...
is Affix::wrap( $lib, 'cb_f_f', [ CodeRef [ [Float] => Float ] ] => Float )->(
    sub {
        is_deeply( \@_, [ $Config{usequadmath} ? -99.3000030517578125 : $Config{uselongdouble} ? -99.3000030517578125 : -99.3000030517578 ],
            '[ Float ]' );
        return -100.5;
    }
    ),
...
is Affix::wrap( $lib, 'cb_d_d', [ CodeRef [ [Double] => Double ] ] => Double )->(
    sub {
        is_deeply( \@_, [ $Config{usequadmath} ? 200.300000000000011368683772161603 : $Config{uselongdouble} ? 200.300000000000011 : 200.3 ],
            '[ Double ]' );
        return 0.4;
    }
    ),
    ( $Config{usequadmath} ? 0.400000000000000022204460492503131 : $Config{uselongdouble} ? 0.400000000000000022 : .4 ), '    => Double';
...
isa_ok my $ptr = Affix::wrap( $lib, 'put_cb_in_struct', [$cb_caller] => Pointer [Void] )->(
    ...
    'Affix::Pointer';
is Affix::wrap( $lib, 'call_cb_in_struct', [ Pointer [Void], Int ] => Int )->( $ptr, 50 ), 100, 'callback stored in a struct triggers correctly';
ok Affix::wrap( $lib, 'free_cb_in_struct', [ Pointer [Void] ] => Int )->($ptr), 'free struct pointer';
#
done_testing;
