...
use Affix qw[:all];
plan skip_all => 'Skipping on MSVC: the gcc-built perl FILE*/PerlIO handles cannot be passed into a cl-built CRT DLL (io-layer ABI mismatch)' if $^O eq 'MSWin32' && ( $ENV{AFFIX_CC} // '' ) =~ /^cl(\b|$)/i;
$|++;
#
subtest simple => sub {
